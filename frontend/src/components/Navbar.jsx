    ...
    <nav className="fixed w-full bg-black/90 backdrop-blur-sm z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6 ml-auto">
            <Link to="/" className="text-gray-300 hover:text-green-500 text-sm font-medium">Home</Link>
            ...
            <div className="flex items-center space-x-4 relative z-[60]">
              ...
              {!user && (
                <Link to="/auth" className="bg-green-500 text-black px-4 py-2 rounded-md text-sm font-medium hover:bg-green-600">
                  Login/Register
                </Link>
              )}
            </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <Link to="/" onClick={() => setIsOpen(false)} className="block text-gray-300 hover:text-green-500 text-base py-2">Home</Link>
            ...
            <ProfileMenu user={user} getUserName={getUserName} handleLogout={handleLogout} />
            {!user && (
              <Link to="/auth" onClick={() => setIsOpen(false)} className="block bg-green-500 text-black px-4 py-2 rounded-md">
                Login/Register
              </Link>
            )}
          </motion.div>
